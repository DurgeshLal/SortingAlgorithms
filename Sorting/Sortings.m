...
-(NSInteger)binarySearchInArray:(NSArray *)array withKey:(NSString *)key havingMin:(NSInteger)min havingMax:(NSInteger)max{
    ...
    if ([array[mid-1] intValue] > [key intValue]) {
        return [self binarySearchInArray:array withKey:key havingMin:min havingMax:mid-1];
    }
    else if ([array[mid] intValue] < [key intValue]){
       return  [self binarySearchInArray:array withKey:key havingMin:mid+1 havingMax:max];
    }
    else{
        return mid;
    }
